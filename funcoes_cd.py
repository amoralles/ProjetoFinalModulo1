def mesmoCartao (cartaoDesejado,cartaoAtual):
    if cartaoDesejado == cartaoAtual:
        return (f'O seu cartão atual já é da categoria {cartaoAtual}')

def novoCartao (cartaoDesejado,renda,score):
    if cartaoDesejado == 'Platinum' and renda >= 5000 and score >= 750:
        return ('O seu perfil é eligível para esse cartão. Em até 10 minutos o cartão estará disponível no app para solicitar.')
    elif cartaoDesejado == 'Black' and renda >= 10000 and score >= 850:
        return ('O seu perfil é eligível para esse cartão. Em até 10 minutos o cartão estará disponível no app para solicitar.')
    elif cartaoDesejado == 'VIP' and renda >= 20000 and score >= 950:
        return ('O seu perfil é eligível para esse cartão. Em até 10 minutos o cartão estará disponível no app para solicitar.')
    else: 
        return ('Infelizmente, você não se enquarda no perfil necessário.')
...
def InformativoBeneficiosPlatinum(cartaoAtual):
    if cartaoAtual == 'Platinum':
        return print( ("""1- Os benefícios do seu cartão são
        2 - Concierge de viagem 24h por dia 
        3 - Seguro Automóvel
        4 - Seguro Médico
        5 - Cashback de 0,5 porcento em todas as compras
        6 - Isenção de anuidade gastando R$ 2.500,00 por mês
        7 - Isenção de rolha em milhares de restaurantes
        """))

def InformativoBeneficiosBlack(cartaoAtual):
    if cartaoAtual == 'Black':
        return print(("""Os benefícios do seu cartão são:
...
def venc (dataFechamento):
    if dataFechamento == 5:
        return ('A data de fechamento da sua fatura é todo dia 15 de cada mês.')
    elif dataFechamento == 8:
        return ('A data de fechamento da sua fatura é todo dia 18 de cada mês.')
    elif dataFechamento == 10:
        return ('A data de fechamento da sua fatura é todo dia 18 de cada mês.')
    elif dataFechamento == 20:
        return ('A data de fechamento da sua fatura é todo dia 27 de cada mês.')
    elif dataFechamento == 25:
        return ('A data de fechamento da sua fatura é todo dia 30 de cada mês.')                    
    

def melhorCompra (dataFechamento):
    if dataFechamento == 5:
        return ('A melhor data de compra é todo dia 6 de cada mês.')
    elif dataFechamento == 8:
        return ('A melhor data de compra é todo dia 9 de cada mês.')
    elif dataFechamento == 10:
        return ('A melhor data de compra é todo dia 11 de cada mês.')
    elif dataFechamento == 20:
        return ('A melhor data de compra é todo dia 21 de cada mês.')
    elif dataFechamento == 25:
        return ('A melhor data de compra é todo dia 26 de cada mês.')
